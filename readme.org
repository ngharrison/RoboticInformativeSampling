...
This will create a directory called =AdaptiveSampling=. Next run =julia AdaptiveSampling/setup.jl=. After that finishes, you should be good to go.
...
1. Navigate into the =AdaptiveSampling/app= directory on the command line
2. Run =julia --project missions/example.jl=
...
1. Navigate into the =AdaptiveSampling/app= directory on the command line
2. Run =julia --project= to open the REPL with the app environment activated
3. Run =incluce("missions/example.jl")=
...
4. Run =rosrun adaptive_sampling ros.jl=
