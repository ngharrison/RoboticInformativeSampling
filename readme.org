...
* Setup
On a command line, just outside the cloned repository, enter the julia REPL by running =julia --project=AdaptiveSampling=. Then enter the interactive Pkg REPL by typing =]= and run the =instantiate= command.

These commands set up the environment and install all needed packages.

** Extra notes
The =--project=<path>= flag activates the project environment on starting the REPL. You can run this manually by =]activate <path>= within the julia REPL. Both of these forms accept the path to the directory that contains the =Project.toml= file, i.e. the root directory of the repository. If you are within a project, you can also simply run =julia --project=@.= with the same effect.

* Quick Start
To run the project in its current state, after navigating into the =AdaptiveSampling/src= directory and starting a project REPL using =julia --project=@.=, run =include("Main.jl")= to run the main file.
